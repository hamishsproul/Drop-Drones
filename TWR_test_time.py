# -*- coding: utf-8 -*-
"""
Created on Thu Aug  8 18:00:54 2024
...
def simulation(TWR, h_max):
    ...
    t_span_ascent = (0, 100)  # max time span for ascent (s)
    ...
    for j in range(0, len(T_ascent)):
        ...
        _, _, rho = air_model(h)
        v = sol_ascent.y[0][j]
        D_ascent[j] = -np.sign(v) * 0.5 * rho * C_D * A * (v**2) # drag opposing velocity
    ...
    def drop_max_thrust_event(t, y):
        ...
        _, _, rho = air_model(h)
        v = y[0]
        T = np.abs(0.5 * rho * C_D * A * (v**2)) # thrust equal to drag
        #print("thrust_max reached")
    ...
    t_span_deceleration = (0, 100)  # time span for landing (s)
    ...
    for j in range(0, len(sol_deceleration.y[1])):
        ...
        D_deceleration[j] = -np.sign(v) * 0.5 * rho * C_D * A * (v**2) # drag opposing velocity
...
for plot_num in range(0, len(h_max_values)):
    ...
    plt.plot(TWR_values, t_test_values[plot_num][:], label=f'{h_max/1000} km')

plt.xlabel('Thrust to Weight Ratio, TWR',fontsize=axis_label_fontsize)
plt.ylabel('Low Gravity Test Time (s)',fontsize=axis_label_fontsize)
plt.grid(True)
...
TWR_values = np.linspace(TWR_min, TWR_max, 100)
t_test_values = np.zeros(len(TWR_values))


for i in range(0, len(TWR_values)):
    TWR = TWR_values[i]
    h_max = 1700
    t_test_values[i] = simulation(TWR, h_max)
#
# Plot the results
...
plt.plot(TWR_values, t_test_values)

plt.xlabel('Thrust to Weight Ratio, TWR',fontsize=14)
plt.ylabel('Low Gravity Test Time (s)',fontsize=14)
plt.grid(True)
plt.tight_layout()